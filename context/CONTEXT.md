...
- **Application D-VTC** (`livrables/applications/0 - D-VTC/`) : Application complète en production sur `d-vtc.fr`. Stack Next.js 14, Supabase, Resend, Vercel. Fonctionnalités livrées : page de réservation client (calculateur de prix temps réel), dashboard chauffeur (stats multi-période, calendrier, réservations, paramètres, changement de mot de passe), notifications temps réel (Supabase Realtime), panel admin (login damiendambassador@gmail.com / admin2026, CRUD chauffeurs complet avec création automatique compte Supabase + email de bienvenue, KPIs financiers), landing page publique avec calculateur de rentabilité vs plateformes. Domaine `d-vtc.fr` actif (Squarespace → Vercel). Emails via Resend avec domaine vérifié `reservations@d-vtc.fr`. Premier client : Patrick (patrick.d.vtc@gmail.com). Tarif commercialisé : 74€/mois. Statut : prêt à commercialiser.
