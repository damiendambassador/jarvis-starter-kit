...
- Poste : Business Development Executive
- Entreprise / Secteur : Edrington (spiritueux : rhum, whisky, gin)
- Missions principales : Prospection chez les cavistes (Nicolas, Repaire de Bacchus, V and B, Nysa). Scope géographique en cours d'élargissement à Paris, Île-de-France et Nord de la France. Nouveau contrat démarrant avec de nouveaux objectifs chiffrés (volumes, comptes) à partir de mi-juin 2026.
...
- **Application D-VTC** (`livrables/applications/d-vtc/`) : Application complète en production sur `d-vtc.fr`. Stack Next.js 14, Supabase, Stripe, Resend, Vercel. Fonctionnalités livrées : page de réservation client (calculateur de prix temps réel), dashboard chauffeur (stats multi-période, calendrier, réservations, paramètres, changement de mot de passe, page "Mes factures"), notifications temps réel (Supabase Realtime), panel admin (connexion via `/connexion`, CRUD chauffeurs complet avec création automatique compte Supabase + email de bienvenue + checkout Stripe + résumé CGV, KPIs financiers, onglet Facturation avec pause/résiliation/renvoi facture), landing page publique avec calculateur de rentabilité vs plateformes. Système de facturation Stripe complet : abonnement 74€/mois, PDF facture auto-généré à chaque paiement, stocké Supabase Storage, envoyé par email au chauffeur. CGV (11 articles) avec modal d'acceptation bloquante à la première connexion. Connexion unifiée `/connexion` (auto-détection chauffeur/admin). Domaine `d-vtc.fr` actif. Emails via Resend (`reservations@d-vtc.fr`). Configuration production complète : migration SQL Supabase, bucket Storage "invoices", produit Stripe Live, webhook enregistré, 10 variables Vercel configurées. Guide de référence : `context/GUIDE-LANCEMENT-DVTC.md`. Premier client : Patrick (patrick.d.vtc@gmail.com). Tarif : 74€/mois. Statut : 100% opérationnel, prêt à commercialiser.
