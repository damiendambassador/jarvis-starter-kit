...
- **Application D-VTC** (`livrables/applications/0 - D-VTC/`) : MVP en cours de développement. Architecture multi-chauffeurs, paiement à bord, notifications email. Phase 1 terminée (scaffolding Next.js 14, schéma Supabase, page de réservation client avec calculateur de prix, page de confirmation, API emails Resend). Maquettes UI générées via Claude Design pour les 8 écrans. Base de données Supabase créée et configurée (projet `oeyfjzccovshgmuhxukq`, région EU West 2, 4 tables + RLS + fonction). `.env.local` configuré, app prête à tester. Phase 2 à venir : dashboard chauffeur (login, réservations, clients, calendrier). Objectif : proposer à un premier client VTC pour générer des revenus via D Embassy.
