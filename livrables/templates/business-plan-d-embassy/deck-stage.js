...
/* â•â•â• THIS PROJECT USES DESIGN COMPONENTS (.dc.html) â•â•â•
 * Reference this stage from your <x-dc> template as an import â€” NEVER as a
 ...
 *     <section data-label="Title" style="...">â€¦</section>
 *     <section data-label="Agenda" style="...">â€¦</section>
 *   </x-import>
 *
...
/* BEGIN USAGE */
/**
 * <deck-stage> â€” reusable web component for HTML decks.
 *
 * Handles:
 *  (a) speaker notes â€” reads <script type="application/json" id="speaker-notes">
 *      and posts {slideIndexChanged: N} to the parent window on nav.
 *  (b) keyboard navigation â€” â†/â†’, PgUp/PgDn, Space, Home/End, number keys.
 *      On touch devices, tapping the left/right half of the stage goes
 *      prev/next â€” taps on links, buttons and other interactive slide
 *      content are left alone.
 *  (c) press R to reset to slide 0 (with a tasteful keyboard hint).
 *  (d) bottom-center overlay showing slide count + hints, fades out on idle.
 *  (e) auto-scaling â€” inner canvas is a fixed design size (default 1920Ã—1080)
 *      scaled with `transform: scale()` to fit the viewport, letterboxed.
 *      Set the `noscale` attribute to render at authored size (1:1) â€” the
 *      PPTX exporter sets this so its DOM capture sees unscaled geometry.
 *  (f) print â€” `@media print` lays every slide out as its own page at the
 *      design size, so the browser's Print â†’ Save as PDF produces a clean
 *      one-page-per-slide PDF with no extra setup.
 *  (g) thumbnail rail â€” resizable left-hand column of per-slide thumbnails
 *      (static clones). Click to navigate; â†‘/â†“ with a thumbnail focused to
 ...
 *      (â‰¤640px), and via the `no-rail` attribute. Rail mutations dispatch
 ...
 *
 * Lifecycle event â€” the component dispatches a `slidechange` CustomEvent on
 ...
 * to the parent â€” just include the #speaker-notes script tag if asked for notes.
 *
 * Authoring guidance:
 ...
 *   - Do NOT set position/inset/width/height on the slide <section> elements â€”
...
(() => {
  ...
  // Label precedence: data-label â†’ data-screen-label (number stripped) â†’ first heading â†’ "Slide".
  ...
  const stylesheet = `
     ...
     * Only the stage/rail hide â€” the black :host background stays, so the
    ...
    /* â”€â”€ Thumbnail rail â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    /* User-driven show/hide (the TweaksPanel toggle) slides instead of
       popping. Transitions are gated on :host([data-rail-anim]) â€” set only
       for the 200ms around the toggle â€” so window-resize and rail-width
       drag (which also call _fit) don't lag behind the cursor. */
    .rail[data-user-hidden] { transform: translateX(-100%); }
    :host([data-rail-anim]) .rail { transition: transform 200ms cubic-bezier(.3,.7,.4,1); }
    :host([data-rail-anim]) .stage { transition: left 200ms cubic-bezier(.3,.7,.4,1); }
    :host([data-rail-anim]) .canvas { transition: transform 200ms cubic-bezier(.3,.7,.4,1); }
    /* transition shorthand replaces rather than merges â€” repeat the base
    ...
    /* Delete-confirm popup â€” matches the SPA's ConfirmDialog layout
    ...
    /* â”€â”€ Print: one page per slide, no chrome â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    @media print {
      ...
      /* :last-child alone isn't enough once data-deck-skip hides the
         trailing slide(s) â€” the last *visible* slide still carries
  ...
  class DeckStage extends HTMLElement {
    ...
    constructor() {
      ...
      // ignore clicks that land inside the menu itself â€” otherwise the
    ...
    connectedCallback() {
      ...
      // next thumbnails â€” the rail has no business rendering inside those
      ...
      // paint has the deck's real typography â€” the :not(:defined) guard in
    ...
    _enableRail() {
      // Idempotent â€” older host builds still post __omelette_rail_enabled.
      // no-rail guard keeps the observers/stylesheet walk off the cheap path
      // for presenter-popup thumbnail iframes (up to 9 per view).
      if (this._railEnabled || this.hasAttribute('no-rail')) return;
      this._railEnabled = true;
      // Per-viewer preference â€” restored alongside rail width. Default on;
      ...
      // refreshes â€” except data-deck-skip, which now arrives from the host
      ...
      this._liveObserver = new MutationObserver((records) => {
        for (const r of records) {
          ...
          // on the thumb wrapper, not the clone) â€” don't mark the slide
      ...
      // Lazy thumbnail materialization â€” clone the slide only when its
      ...
      this._onTweakChange = () => {
        ...
        this._tweakTimer = setTimeout(() => {
          ...
          // One getComputedStyle for the whole batch â€” each
      ...
      this._snapshotAuthorCss();
      // Build the rail now that it's enabled â€” slotchange already fired,
     ...
     *  thumbnail's nested shadow root adopts â€” so author CSS styles the
     *  cloned slide content without touching this component's chrome.
     *  Cross-origin sheets throw on .cssRules â€” skip them. Re-callable:
    ...
    _snapshotAuthorCss() {
      ...
      // Rewrite :root â†’ :host and mirror <html>'s data-*/class/lang onto
      ...
      const authorCss = Array.from(document.styleSheets).map((sh) => {
        ...
        // form, so any compound on :root â€” [attr], .class, #id, :pseudo â€”
    ...
    _syncThumbHostAttrs(host, cs) {
      ...
      // setAttribute overwrites but can't delete â€” an attr removed from
      ...
      // The :rootâ†’:host rewrite in _snapshotAuthorCss pins each custom
      // property to its stylesheet default on the thumb host, shadowing
      // the live value that would otherwise inherit. Tweaks can write the
      // live value on any ancestor â€” <html>, <body>, a wrapper div, the
      // deck-stage element â€” so read it as the *computed* value at
    ...
    _render() {
      ...
      overlay.innerHTML = `
        <button class="btn prev" type="button" aria-label="Previous slide" title="Previous (â†)">
          <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M10 3L5 8l5 5"/></svg>
        </button>
        <span class="count" aria-live="polite"><span class="current">1</span><span class="sep">/</span><span class="total">1</span></span>
        <button class="btn next" type="button" aria-label="Next slide" title="Next (â†’)">
      ...
      // (ramped by edge proximity) is enough â€” no rAF loop needed.
      ...
      // Rail resize handle â€” drag to set --deck-rail-w, persisted to
      ...
      // Delete-confirm dialog â€” mirrors the SPA's ConfirmDialog layout.
    ...
    /** @page must live in the document stylesheet â€” it's a no-op inside
    ...
    _syncPrintPageRule() {
      ...
      tag.textContent =
        ...
        // never captures mid-entrance â€” pairs with the beforeprint handler
     ...
     *  break-after (see the @media print comment above â€” :last-child
    ...
    _restoreIndex() {
      ...
      // the iframe src. No hash â†’ slide 1; the deck itself keeps no position
      // state across loads.
    ...
    _applyIndex({ showOverlay = true, broadcast = true, reason = 'init' } = {}) {
      ...
      // tap, external goTo) â€” the only time we *don't* want the rail to
    ...
    _flashOverlay() {
      ...
      // mode â€” suppress the nav footer entirely (both hover and slide-change
    ...
    _fit() {
      ...
      // geometry â€” the scaled canvas is in shadow DOM, so the exporter's
    ...
    _onResize() {
      this._fit();
      // Crossing the narrow-viewport breakpoint reveals the rail â€” rerun the
    ...
    _onMessage(e) {
      ...
      // slotchange â€” setAttr and refusal don't fire one. On refusal,
      ...
      // whether the Tweaks panel itself is open â€” closing the panel
      // doesn't change rail visibility. Persists alongside rail width.
      if (d && d.type === '__deck_rail_visible' && typeof d.on === 'boolean') {
        if (d.on === this._railVisible) return;
        this._railVisible = d.on;
        try { localStorage.setItem('deck-stage.railVisible', d.on ? '1' : '0'); } catch (e) {}
        // Arm the transition, commit it, then flip state â€” otherwise the
    ...
    _syncRailHidden() {
      ...
      // presentation mode, and the host's Preview segment â€” instant, no
      ...
      // translateX hide leaves thumbs (tabIndex=0) in the tab order â€”
      // inert keeps them unfocusable while the rail is off-screen.
      this._rail.inert = hard || !this._railVisible;
    }

    _onTap(e) {
      // Touch-only â€” keyboard + the overlay toolbar cover nav on desktop.
      ...
      // e.target.closest) so we see through open shadow roots â€” a <button>
    ...
    _onKey(e) {
      ...
      // is left to the focused button's native activation so Tabâ†’Cancel
      // â†’Enter activates Cancel, not the window-level confirm path.
    ...
    // â”€â”€ Thumbnail rail â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    //
    ...
    _renderRail() {
      ...
      // Orphans â€” slides removed since last render.
      bySlide.forEach((t) => {
      ...
      if (prevTops.size) {
        ...
        if (moved.length) {
          ...
          // on â€” otherwise the browser coalesces both style writes and
          // nothing animates.
    ...
    _makeThumb(slide) {
      ...
      // â†‘/â†“ step through the rail when a thumb has focus. _go clamps at the
      // ends and _applyIndexâ†’_syncRail scrolls the new current thumb into
      // view; we move focus to it (preventScroll â€” _syncRail already
    ...
    _materialize(entry) {
      ...
      // is paint/composite â€” lazy+async keeps that off the main thread.
      ...
      // preserved so layout-wrapper elements (<my-column><h2>â€¦</h2>)
      ...
      // querySelectorAll('*') returns descendants only â€” a custom-element
      // slide root (<my-slide>â€¦</my-slide>) would slip through and upgrade
      ...
      // Once materialized the IO callback is a no-op early-return â€”
      // unobserve so scroll doesn't keep firing it.
      if (this._railObserver) this._railObserver.unobserve(entry.frame);
    }

    /** Re-clone a single thumb (live-update path). No-op if the thumb
     *  hasn't been materialized yet â€” it'll pick up current content when
    ...
    _scaleThumbs() {
      ...
      // display:none (noscale / no-rail / presenting / print) â€” leave the
     ...
     *  the host owns the light DOM â€” handlers emit a dc-op only and the
    ...
    _emitDcOp(op, slide, lock, newIndex) {
      // Slide index (template/script/style filtered â€” same as
      ...
      // <div class="sc-host-x" data-dc-tpl="N"> host â€” the stamp is on the
